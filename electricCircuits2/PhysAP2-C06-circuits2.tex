...
For example, Figure~\ref{fig:multiloop1} shows a simple example with two
...
where a capacitor and a resistor are connected in series, as shown in
Fig.~\ref{fig:discharing-capacitor}.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[american voltages,scale=.8]
    \draw[thick] (0,0) to[C=$C$] (0,3)--(3,3) to[R=$R$] (3,0)--(0,0);
