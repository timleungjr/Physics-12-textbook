...
\begin{figure}[ht]
  ...
  \begin{subfigure}{.35\textwidth}
    ...
    \end{tikzpicture}
  \end{subfigure}
  \caption{Current and voltage from an alternating source}
\end{figure}
...
\end{equation*}
Noting that $\frac{\Delta q}{\Delta t}$ is the electrical current through the
circuit, we have a final expression for power through a circuit:
\begin{important-equation}
  P=IV
\end{important-equation}
%  \begin{center}
...
In circuit analaysis, \emph{parallel} means devices that are connected side by
side to each other. In the case of resistors in parallel, one such example is
shown in Fig.~\ref{fig:parallel-resistors}.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[thick]
...
From the junction law, we know that the total current is the current through all
the parallel resistors, which we can then express in terms of voltage and
resistance using Ohm's law:
\begin{equation*}
  I=I_1+I_2+I_3+\cdots=\frac{V_1}{R_1}+\frac{V_2}{R_2}+\frac{V_3}{R_3}+\cdots
  \label{eq:parallel1}
\end{equation*}
From the voltage law, we know that the voltage across the parallel resistors
must be the same, i.e.\ $V_1=V_2=V_3=\cdots=V$, therefore we can factor out the
voltage term in Eq.~\ref{eq:parallel1}, and re-write in terms of an
\emph{equivalent} resistance $R_p$:
\begin{equation*}
  I=\frac V{R_p}=V\left(\frac1{R_1}+\frac1{R_2}+\frac1{R_3}\cdots\right)
\end{equation*}
From this equation, we find the parallel equivalent resistance of $N$ resistors
connected in parallel: The inverse of the equivalent resistance $R_p$ is the
sum of the inverses of the individual resistances $R_i$:
\begin{important-equation}
  \frac1{R_p}=\sum_{i=1}^N\frac1{R_i}
\end{important-equation}
Increasing the number of resistors in parallel \emph{decreases} $R_p$, as
shown in Fig.~\ref{fig:parallel-resistance} because it effectively increases
the cross section area that the current passes through. Conversely, removing
resistors from a parallel configuration increases the overall resistance.
Also, the parallel equivalent resistance is \emph{always} lower than the any
individual resistor.
\begin{figure}[ht]
  \centering
  \begin{subfigure}{.3\linewidth}
    ...
    \vspace{.55in}$R_\text{eq}=R$
    \caption{A single resistors}
  \end{subfigure}
  \begin{subfigure}{.3\linewidth}
    \centering
    \begin{tikzpicture}[thick]
  ...
  \begin{subfigure}{.3\linewidth}
    ...
    \vspace{.1in}$R_\text{eq}=\frac R3$
    \caption{Three resistors in parallel}
  \end{subfigure}
  \caption{Decreasing equivalent resistance when more resistors are connected
    in parallel}
  \label{fig:parallel-resistance}
\end{figure}
...
\begin{figure}[ht]
  ...
  \end{tikzpicture}
  \caption{Circuit diagrams of three resistors connected in series}
\end{figure}
...
\begin{example}
  ...
  \begin{minipage}{.43\linewidth}
    \begin{tikzpicture}[thick]
      \draw (5,3) to[battery,l_=$\mathcal E$] (0,3)--(0,0)--(.5,0)
      to[bulb=$R_2$] (3,0)--(5,0) to[bulb=$R_4$] (5,3);
      \draw (.5,0)--(.5,1.5) to[bulb=$R_1$] (3,1.5)--(3,0);
      \draw (.5,0)--(.5,-1.5) to[bulb=$R_3$] (3,-1.5)--(3,0);
    \end{tikzpicture}    
  \end{minipage}
  \begin{minipage}{.55\linewidth}
    ...
    \item If $R_3$ is disconnected from the circuit so that only 3 light bulbs,
      ...
      %(Hint: You may show calculations, but if you understand resistors in
      %parallel and series, calculations are not necessary.)
    \end{enumerate}
  \end{minipage}
