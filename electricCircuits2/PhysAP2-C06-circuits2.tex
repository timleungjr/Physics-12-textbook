...
In Chapter~\ref{chapter:capacitor}, we learned that the capacitor is a device
that stores energy in its electric field. But whereas
Chapter~\ref{chapter:capacitor} dealt primarily with how we define the
...
final \emph{steady-state} solution for $t\rightarrow\infty$, when the current
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{.49\textwidth}
    ...
    \end{tikzpicture}
    \caption{Steady-state circuit as $t\longrightarrow0$}
  \end{subfigure}
