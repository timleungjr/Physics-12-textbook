...
\begin{important-equation}
  E_0=mc^2
\end{important-equation}
The \textbf{total energy} of an object has is
\begin{important-equation}
  E_T=m'c^2=\gamma mc^2
\end{important-equation}
The difference between total energy and rest energy is the kinetic energy:
\begin{important-equation}
...
%  \end{tabular}
%\end{center}
This equation is derived by first squaring the equation for relativistic
momentum (Eq.~\ref{eq:relativistic-momentum}), and substittuting the expression
for $\gamma$:
\begin{equation*}
  p=\gamma mv \quad\to\quad
...
\begin{important-equation}
  E=pc\quad\text{(masslass particle)}
\end{important-equation}
...
\begin{center}
  \begin{tabular}{cc}
    \textbf{Relativistic:} & \textbf{Newtonian:}\\
    $\displaystyle K=(\gamma-1)mc^2$ &
    $\displaystyle K=\frac12mv^2$ \\
  \end{tabular}
...
\end{equation}
In the case of relativistic kinetic energy, we want to do this same series
expansion for the Lorentz's factor:
\begin{equation*}
  \gamma=\left[1-\left(\frac vc\right)^2\right]^{-\frac 12}
\end{equation*}
