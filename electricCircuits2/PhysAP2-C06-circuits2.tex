...
For example, Figure~\ref{fig:multiloop1} shows an example with two
voltage sources and three resistors. This circuit cannot be reduced.
...
In Chapter~\ref{chapter:capacitors}, we learned that the capacitor is a device
that stores energy in its electric field. But whereas
Chapter~\ref{chapter:capacitors} dealt primarily with how we define the
capacitance, here, we are interested in how a capacitor can be used in a
circuit.

When a capacitor is charged, that is, when there is a charge of $\pm Q$ on
either side of the capacitor ($+Q$ on one side, and $-Q$ on the other), it acts
as an energy storage device, it generates a voltage of
\begin{displaymath}
  V_c=\frac QC
\end{displaymath}
that is proportional to the charge on the capacitor plates. The capacitance $C$
depends on the geometry of the capacitor, and whether a dielectric material is
inserted  between the two sides of the capacitor. This voltage $V_c$ can be
used to drive a current in a circuit. However, unlike a battery, where the
voltage is determined \emph{chemically}, in a capacitor, the voltage would
change as the capacitor is charging or discharging. This means that the current
in the circuit would not be steady (i.e.\ current is a function of time).

The analysis of circuits that have both resistors and capacitors, called an
``RC circuit'' focuses on finding two specific operating conditions: an
\emph{initial condition}, for when the current begins to flow at $t=0$, and a
final \emph{steady-state} solution for $t\to\infty$, when the current
...
%  V_c=\frac QC
%\end{equation*}
%where $C$ is the capacitance. The voltage can be used to drive a current
%in the circuit. However, unlike in a battery, where a nearly-constant voltage is
%generated , in a capacitor, voltage can vary depending on
...
\begin{important-equation}
  C_p=\sum_{i=1}^N C_i\quad\quad\text{(parallel capacitors)}
...
\begin{important-equation}
  \frac1{C_s}=\sum_{i=1}^N\frac1{C_i}\quad\quad\text{(series capacitors)}
\end{important-equation}
...
combine them in to an \textbf{RC circuit}. An $RC$ circuit merely refer to
any circuit that contain both resistors and capacitors, but does not necesarily
indicate a specific arrangment of the devices.
%  \begin{center}
...
the capacitor would have been $+Q_0$ on one side, and $-Q_0$ on another.)
...
\end{figure}
%The capacitor initially charged with a
%charge of $+Q$ on one slide, and $-Q$ on the other. The ini
Initially, the switch is open, and no current flows in the circuit. At this
time, the capacitor has an initial charge $Q_o$ across the plates (i.e.\ $+Q$
on one side, and $-Q$ on the other), and therefore a voltage of $V_0=Q_0/C$. At
$t=0$, the switch is closed, completing the circuit. Current---driven by the
voltage in the capacitor plates---begin to flow at $t=0$. The initial current
$I_0$ through the circuit is based on Ohm's law:
\begin{displaymath}
  I_0=\frac{V_0}R
\end{displaymath}
As current flows, charge $Q(t)$ across the capacitor decreases in time, and
therefore voltage
\begin{equation*}
  V_c(t)=\frac{Q(t)}C
\end{equation*}
across the capacitor decreases. Since this voltage drives the current
\begin{equation*}
  I(t)=\frac{V_c(t)}R=\frac{Q(t)}{RC}
\end{equation*}
also decreases. But since current decreases, $Q$ also decreases more slowly.
...
%  Q(t)=Q_0e^{-t/\tau}
%\end{important-equation}  
%>>>>>>> 317105678c5ffcc4eff105474c2ee9d8a1facdb9
...
As $t\\to\infty$, the voltage across the capacitor:
$V_c\\to\mathcal E$, and the charge across the capacitor:
$Q(t)\\to C\mathcal E$. The current through the circuit:
$I\\to 0$ begin to accumulate on the capacitor.
%%\begin{itemize}
...
cannot be simplified into the configurations shown in the previous sections.
Fig.~\ref{fig:general-rc-circuit} shows such an example.
\begin{figure}[ht]
  \centering
  ...
  \label{fig:general-rc-circuit}
\end{figure}
In this case, the capacitor in the circuit is initially uncharged. We can find
the current through, and the voltage across, each element
\begin{enumerate}
\item Immediately after the switch is closed (the initial condition at $t=0$)
\item A long time after the switch is closed (the steady-state solution for
  $t\to\infty$)
\end{enumerate}
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{.49\textwidth}
    ...
    \end{tikzpicture}
    \caption{Steady-state circuit as $t\to\infty$}
  \end{subfigure}
...
But we can also find the behaviour of the circuit \emph{between} $t=0$ and
$t=\infty$ as well. In this case, we cannot find the time constant for such a
circuit, but we do know that the change would be an exponential decay.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=1.5,american voltages,thick]
    ...
    to[R=\SI4\ohm](2,2)--(4,2) to[C=\SI6{\micro\farad}] (4,0)--(0,0);
    \draw (3,0) to[R=\SI8\ohm] (3,2);
    \begin{scope}[red]
      \draw[vectors] (0,1.3)--(0,2)--(.4,2);
      \draw[axes] (-2.5,2)--+(1.5,0) node[right]{$t$};
      \draw[axes] (-2.5,2)--+(0,1.8);
      \draw[thick] (-2.5,3.5)--+(-.1,0) node[left=-3]{\scriptsize\SI3\ampere};
      \draw[thick] (-2.5,2.5)--+(-.1,0) node[left=-3]{\scriptsize\SI1\ampere};
      \draw[thick,dashed] (-2.5,2.5)--+(1.5,0);
      \draw[thick,domain=-2.5:-1] plot(\x,{exp(-4*(\x+2.5))+2.5});
      \node[above] at (-1.75,3.7){$I_\text{batt}$ and $I_{\SI4\ohm}$};
    \end{scope}
    
    \begin{scope}[violet]
      %\draw[vectors] (0,1.3)--(0,2)--(.4,2);
      \draw[axes] (0,3)--+(1.5,0) node[right]{$t$};
      \draw[axes] (0,3)--+(0,1.8);
      \draw[thick] (0,4.5)--+(-.1,0) node[left=-3]{\scriptsize\SI{12}\volt};
      \draw[thick] (0,3.5)--+(-.1,0) node[left=-3]{\scriptsize\SI4\volt};
      \draw[thick,dashed] (0,3.5)--+(1.5,0);
      \draw[thick,domain=0:1.5] plot(\x,{exp(-4*\x)+3.5});
      \node[above] at (.75,4.5){$V_{\SI4\ohm}$};
    \end{scope}

    \begin{scope}[orange]
      \draw[vectors] (3,2)--+(0,-.6);
      \draw[axes] (2.5,2.5)--+(1.5,0) node[right]{$t$};
      \draw[axes] (2.5,2.5)--+(0,1.4);
      \draw[thick] (2.5,3.5)--+(-.1,0) node[left=-3]{\scriptsize\SI1\ampere};
      \draw[thick,dashed] (2.5,3.5)--+(1.5,0);
      \draw[thick,domain=2.5:4] plot(\x,{1-exp(-4*(\x-2.5))+2.5});
      \node at (3.25,4){$I_{\SI8\ohm}$};

      \draw[axes] (5,2.5)--+(1.5,0) node[right]{$t$};
      \draw[axes] (5,2.5)--+(0,1.4);
      \draw[thick] (5,3.5)--+(-.1,0) node[left=-3]{\scriptsize\SI8\volt};
      \draw[thick,dashed] (5,3.5)--+(1.5,0);
      \draw[thick,domain=5:6.5] plot(\x,{1-exp(-4*(\x-5))+2.5});
      \node at (5.75,4){$V_{\SI8\ohm}$ and $V_c$};
    \end{scope}
    
    \begin{scope}[blue]
      \draw[vectors] (3.5,2)--(4,2)--(4,1.2);
      \draw[axes] (5.5,.5)--+(1.5,0) node[right]{$t$};
      \draw[axes] (5.5,.5)--+(0,1.4);
      \draw[thick] (5.5,1.5)--+(-.1,0) node[left=-3]{\scriptsize\SI3\ampere};
      \draw[thick,domain=5.5:7] plot(\x,{exp(-4*(\x-5.5))+.5});
      \node[above] at (6.25,1.5){$I_c$};
    \end{scope}
  \end{tikzpicture}
\end{figure}
