...
\begin{figure}[ht]
  \centering
  \pic{.6}{quantumMechanics/graphics/Glowing_metal}
  \caption{Metals glow when heated}
  \labwel{fig:glowing-metal}
\end{figure}
...
%    \column{.75\textwidth}
\begin{itemize}
\item The concept was coined by 
%\end{itemize}
%
%
%
...
%    \column{.75\textwidth}
%    \begin{itemize}
\item The object is in thermal equilibrium (recall thermodynamics from
  Classes \#1 and \#2); all of the absorbed  energy is then
  \emph{immediately} radiated back as EM radiation
\item The spectral distribution depends only on temperature
\item A blackbody at room temperature appears black, as most of the
  radiation is infrared and cannot be perceived by the human eye
\item Radiation spontaneously emitted by many ordinary objects can
...
As temperature of the blackbody increases: Radiation intensity increases (i.e.\
the object glows brighter). The peak shifts towards shorter wavelengths (i.e.\
colour become more bluish). The peak of the curve is predicted by
\textbf{Wien's displacement law} (black line)
...
\textbf{Rayleigh-Jeans law}. Based on established physics: standing waves,
Maxwell's unified laws of electrodynamics, and thermodynamics.
      %\footnote{If you are interested, and
      %have background in calculus, you can find a fairly simple explanation
      %via \href{https://hyperphysics.gsu.edu/hbase/mod6.html}
      %{hyperphysics.gsu.edu/hbase/mod6.html})}

Rayleigh-Jeans law predict that the energy per unit volume per unit wavelength
is:
\begin{equation}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      \addplot[black] {planck(x,4000)};
      \addplot[black] {planck(x,5000)};
      \addplot[red,ultra thick,domain=1000:3500] {rayleighjeans(x,5000)};
    \end{axis}
      
    % LABELS
    \node[scale=.7] at (3,.85) {\SI{3000}\kelvin};
    \node[scale=.7] at (2.6,2) {\SI{4000}\kelvin};
...
but shorter wavelengths (e.g.\ ultraviolet waves) seem to have infinite
intensity. This was known as the \emph{ultraviolet catastrophe}.


The equation that \emph{best} predicted the spectral radiance for blackbody
radiation was \textbf{Wien's distribution law}.
    %which predicts the peaks wavelength of
    %the spectral distribution curve, and
It fits experimental data for shorter wavelengths, but not for lower
frequencies (low wavelengths).
\begin{itemize}
\item This law is an ``empirical function'' to fit the data
\end{itemize}
...
1927} studied x-ray scattering by free electrons
\begin{itemize}
\item ``Free electrons'' are actually valence electrons in atoms that have
  \emph{very} low ionization energy, so any EM radiation will knock them off
  the atom
\item After hitting the electrons, the x-ray radiation is scattered
\item Those electrons ejected from the atoms are called \textbf{recoil
  electrons}
\end{itemize}
\begin{center}
  \input{quantumMechanics/compton}
\end{center}
...
\begin{equation}
  \boxed{p=\frac Ec=\frac{hf}c=\frac h\lambda}
\end{equation}
...
are conserved:
\begin{equation}
  \vec p_i=\vec p_e + \vec p_f\quad\text{and}\quad
  E_i = E + E_f
\end{equation}
\begin{center}
  \begin{tikzpicture}
