...
%      \draw[vectors,red] (0,0)--(2,0) node[right]{$\bm F_q$};
%      \draw[vectors,blue] (8,0)--(6,0) node[left]{$\bm F_q$};
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.4]
    ...
    \draw[vectors,blue] (0,.7)--(0,3) node[right]{$\bm F_1$};
    \shade[ball color=blue] (0,8) circle (.6) node[white]{$q_1$};

    \draw[vectors,orange](.7,0)--(4,0) node[right]{$\bm F_2$};
    ...
    \draw[vectors,rotate=-35](.7,0)--(2.7,0) node[right]{$\bm F$};
  \end{tikzpicture}
...
\end{equation*}
Gravitational field $\bm g$ is function of two variables: source $m_S$ and
...
$\bm g$, you can also find the force that acts on a mass:
$\bm F_g=m\bm g$.
...
\end{equation*}
A source charge $q_s$ creates an ``electric field'' ($\bm E$) with a
...
\end{important-equation}
where $r$ is the distance from the source point charge.
...
\end{important-equation}
where $\hat{\bm r}$ is the \emph{outward radial direction}. If the source
charge $q_s$ is positive, the direction of the electric field points away from
the charge, (in the $\hat{\bm r}$ direction with a magnitude speicified in
...
an electrostatic force $\bm F_q$ on the charge, regardless of what created
the electric field.
\begin{important-equation}
  \bm F_q=q\bm E
\end{important-equation}
The equation is similar to the law of gravitation ($\bm F_g=m\bm g$),
but with $\bm F_q$ replacing $\bm F_g$, $q$ replacing $m$, and $\bm
E$ replacing $\bm g$. If a positive charge ($q>0$) is inside the field,
$\bm F_q$ is in the \emph{same} direction as $\bm E$; if it is a
negative charge ($q<0$), $\bm F_q$ is in the \emph{opposite} direction as
$\bm E$.
...
charge, i.e.:
\begin{important-equation}
  \bm E=\sum_{i=1}^N \bm E_i
  =\bm E_1+\bm E_2+\bm E_3+\bm E_4+\cdots+\bm E_N
\end{important-equation}
Once the total electric field is known, we can use the equation
$\bm F_q=q\bm E$ without knowing \emph{what} created $\bm E$.
