...
\begin{questions}
  ...
  \begin{center}
    \begin{tikzpicture}[thick]
      \draw[dashed,fill=gray!10] circle (1);
      \node at (1.2,-.5) {$X$};
      \draw (0,2.5) circle (.1) node[above]{$e^-$};
      \draw[vectors] (0,2.4)--(0,1);
    \end{tikzpicture}
  \end{center}
  \begin{choices}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.3]
      \draw[thick] rectangle (.2,2);
      \draw[thick] (1,0) rectangle (1.2,2);
      \draw[vectors] (.1,2)--(.1,3) node[above]{$I_A$};
      \draw[vectors] (1.1,2)--(1.1,3) node[above]{$I_B$};
    \end{tikzpicture}
  \end{center}
  \begin{choices}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.9]
      \draw[line width=5,gray] (0,0)--(0,5);
      \draw[line width=5,gray] (2.1,0)--(2.1,5);
      \foreach \x in {.3,.6,...,2}{
        \foreach \y in {.2,.5,...,5} \node at (\x,\y) {$\times$};
      }
      \draw[vectors] (1.05,-.5)--(1.05,5.5);
  ...
  \begin{choices}
    \choice
    \begin{tikzpicture}
      \draw[very thick] circle (.2) node{$p$};
      \draw[vectors] (.2,0)--(1.2,0);
      \draw[vectors] (-.2,0)--(-1.2,0);
    \end{tikzpicture}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.2]
      ...
      \fill[thick] (-.5,3) circle (.08) node[above] {$e$};
      \draw[vectors] (-.5,3)--(-.5,2) node[below]{$v$};
    \end{tikzpicture}
  \end{center}
  \begin{choices}
    \choice The force is directed towards the right.
    \choice The force is directed towards the left.
...
%      \draw[thick,->](-.8,0) arc(180:135:.8) node[above]{$I$};
%      \node at (1.3,1) {$B$};
%    \end{tikzpicture}
%  \end{minipage}
...
%      \choice The wire loop will flip on a horizontal axis\\
%      through its centre.
...
%  
  \uplevel{
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.9]
        \foreach \x in {0,...,3}{
          \foreach \y in {0,...,3} \fill (\x,\y) circle (.08);
        }
        \fill[thick] (1.5,1.5) circle (.05);
        \draw[vectors] (1.5,1.5)--(2.5,1.5) node[midway,above]{$r$};
        \draw[dashed] (1.5,1.5) circle (1);
        \draw[fill=white] (1.5,2.5) circle (.2);
        \draw[vectors] (1.5,2.5)--(.5,2.5) node[left]{$v$};
      \end{tikzpicture}
    \end{center}
  }
...
%    \choice The magnitude of the magnetic field produced by the magnet changes.
%    \choice The angle between the plane of the coil and the magnetic field
%    produced by the magnet change.
%    \choice The strength of the magnetic field in the plane of the coil changes.
%  \end{choices}
  \newpage
...
%  \question A loop of wire is positioned perpendicular to a magnetic field, as
%  shown in the figure.
%  \begin{center}
%    \begin{tikzpicture}
%      \foreach\x in {0,...,3}{
...
%  \end{center}
%  \begin{parts}
%    \part Write the magnetic flux equation. Describe three unique methods
%    for inducing current by changing one of the terms in the magnetic
%    flux equation.
%    
%    \part Describe how the wire loop could be moved without inducing an
%    electric current in the wire.
%    
%    \part If the wire is held still and the magnetic field increases in
%    strength, which direction will current be induced in the wire?
%  \end{parts}
%  \newpage
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[scale=1.2]
      \begin{scope}[thick]
        ...
        \draw (1.1,1) circle (.1) node[above=3]{$m$} node[below left]{$q$};
        \draw[vectors] (1.2,1)--(2,1) node[right]{$v$};
      \end{scope}
    \end{tikzpicture}
  }

  \question A positively charged particle ($q$) of mass $m$ travels
  horizontally, with a velocity of $v$, through the centre of two capacitor
  plates. The plates are separated by a distance of $d$ and connected to a
  battery of potential difference ($\mathcal E$), as shown in the figure.
  \begin{parts}
    \part On the diagram above, sketch the electric field between the plates.
    \vspace{.2in}
    ...
    positively charged particles with a velocity of $v$ to travel straight. What
    happens to a particle traveling at $2v$? Will it travel straight, or
    ...
    charged particles with a velocity of $v$ to travel straight. What happens
  ...
  \uplevel{
    ...
    \begin{tikzpicture}
      \foreach\x in {0,...,5}{
        \foreach\y in {0,...,5} \fill (\x,\y) circle (.08);
      }
      \draw[thick] (6,.5) --(-2,.5) to[R,l=$R$] (-2,4.3)--(6,4.3);
      \draw[thick,<->] (5.7,.5)--(5.7,4.3) node[midway,fill=white]{$y$};
      \fill (1.4,0.3) rectangle (1.6,4.6);
      \draw[vectors] (.5,2.5)--(2.5,2.5) node[right]{$\vec v$};
      \draw[thick,<->] (-2,-.3)--(1.5,-.3) node[midway,fill=white]{$x$};
      \node at (4.5,5.4){$\vec B$};
    \end{tikzpicture}
  }
  \question Two long parallel wires, separated by a distance of $y$, pass
  through a region of magnetic field ($\vec B$). The two wires are connected
  by a resistor ($R$) and a metal bar, separated by a distance of $x$, to
  produce a circuit loop, as shown in the figure. The metal bar slides along
  the wires to the right at a velocity of $\vec v$.
  \begin{parts}
    \part Calculate the induced emf ($\mathcal E$) in the bar in terms of the
    given quantities.
    \vspace{\stretch1}
  ...
  \begin{parts}
    ...
    \part In what direction must the magnetic field $\mathbf B$ point in order
    to create the force $\mathbf F$? Explain your reasoning.
    \vspace{\stretch1}
  ...
  \begin{parts}
    ...
    \part Using the straight line you drew in part (\ref{partd}), calculate the
    value $B$ for the magnetic field if $m$ is \SI{.019}{\kilo\gram}, $L$ is
