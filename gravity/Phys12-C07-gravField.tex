...
\begin{figure}[ht]
  ...
  \label{fig:gravity1}
\end{figure}
The magnitude of the \textbf{gravitational force} ($\bm F_g$) between two
masses is described by the \textbf{law of universal gravitation}. This force is
...
where $G=\SI{6.67e-11}{N.m^2/kg^2}$ is the
\textbf{universal gravitational constant}. This equation is derived based on
the second law of motion and Kepler's laws of planetary motion (discussed in
\ref{section:keplers-laws} later in this chapter).
...
\emph{minimum} speed at which the object would not fall back to Earth
because of gravity, called the \textbf{escape speed}. To derive the escape
speed, we assume that
...
\textbf{Orbital kinetic energy:} We can obtain the orbital kinetic energy of an
object in a \emph{perfectly circular orbit} by applying the equation for
orbital speed (Eq.~\ref{eq:orbital-velocity}) and the definition of kinetic
energy (Eq.~\ref{eq:KE-definition}):
\begin{equation}
  K_\text{orbit}=\frac12mv_\text{orbit}^2=\frac12m
  \left(\sqrt{\frac{GM}r}\right)^2
\end{equation}
which gives us this equation:
\begin{important-equation}
  K_\text{orbit}=\frac{GMm}{2r}
  \label{eq:orbital-KE}
\end{important-equation}
...
gravitational potential energies while in orbit. Substituting the expressions
for $K_\text{orbit}$ (Eq.~\ref{eq:orbital-KE}) and $U_g$
(Eq.~\ref{eq:GPE-general}) derived for objects in circular orbit:
\begin{equation}
  E_\text{orbit}=
  K_\text{orbit}+U_g =
  \frac{GMm}{2r} +%{K_\text{orbit} +
  \left(-\frac{GMm}r\right) %}_{U_g}=
\end{equation}
...
\begin{important-equation}
  E_\text{binding}=E_\infty-E_\text{orbit}=\frac{GMm}{2r}
  %-K_\text{orbit})=K_\text{orbit}
\end{important-equation}
...
We can see that there are simple relationships betweeen orbital kinetic energy,
gravitational potential energy, total energy and escape energy:
%Gravitational potential energy is $-2$ times the orbital kinetic energy:
\begin{equation}
  K_\text{orbit} = \frac12U_g = -E_\text{orbit} = E_\text{binding}
\end{equation}
