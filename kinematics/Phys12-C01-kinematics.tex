...
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[rotate around={-10:(9,0)}]
    \draw[thick] (0,0)--+(8.8,0);
    \draw[thick] (.3,.1) rectangle +(.8,.2);
    \draw[thick,fill=white] (.5,.1) circle (.1);
    \draw[thick,fill=white] (.9,.1) circle (.1);
    \foreach \x in {.7,1.2,...,8.5} \draw[thick](\x,0)--+(0,-.15);
  \end{tikzpicture}
\end{figure}
...
\subsubsection*{Example: Snell's Law}
For example, to find the index of refraction of a material using
...
\subsubsection*{Example: Period of Oscillation of a Simple Pendulum}
...
\begin{wrapfigure}{r}{.34\textwidth}
  ...
  \end{tikzpicture}
  \caption{The $xy$-plane is a 2D Cartesian coordinate system}
\end{wrapfigure}
The coordinate system for two dimensional motion is the Cartesian $xy$-plane,
as we have discussed briefly in the previous chapter. The $x$ and $y$ axes are
\textbf{orthogonal} to each other.
%\footnote{Two vectors are orthogonal when they are pependicular to
%each other}.
This means that motion along the $x$ axis is independent of motion along the
$y$ axis. Mathematically, we call this \textbf{linear independence}.
...
\section{Describing a 2D Vector} %On The $xy$-Plane}

Once a 2D coordinate system has been set, \emph{any} vector inside the
coordinate system is a 2D vector. Like vectors in 1D, vectors in higher
dimensions are also represented as arrows. The magnitude of the vector
is the length of the arrow; the direction of the vector can be described by the
angle it makes with the $x$-axis. One way to describe \emph{any} 2D vector is
to explicitly state the magnitude of the vector, with the direction inside a
bracket:
\begin{center}
  magnitude [direction]
\end{center}
In introductory-level physics courses, this is often the preferred way of
expressing 2D vectors, because it can be understood intuitively by a novice
physics student. However, in high-level physics courses, it is often more
convenient to express 2D vectors in component form. Fig.~\ref{fig:2d-vectors}
shows four examples of 2D vectors on the $xy$-plane. Describing vectors in
component form is discussed in Section~\ref{section:component-form}.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[scale=.7]
    ...
    \draw[axes] (-1.2,3) arc (0:225:.8) node[pos=.75,left]{\SI{225}\degree};
    \draw[vectors,red] (2,-1)--+(2,0) node[below]{$\bm C$};
    
    \draw[vectors,red] (-1,-1)--+(0,-1.5) node[right]{$\bm D$};
    \draw[axes] (-.2,-1) arc (0:270:.8) node[pos=.75,left]{\SI{270}\degree};
  \end{tikzpicture}
  \caption{Four examples of 2D vectors on the $xy$-plane}
...
$-y$ axis. We can express these vectors by directly stating their magnitudes and
directions:
\begin{align*}
  \bm A &= 5\;[\SI{37}\degree]\\
  \bm B &= 2.82\;[\SI{135}\degree]\\
  \bm C &= 2\;[\SI0\degree]\\
...
components $A_x$ and $A_y$ can be easily obtained using the sine and cosine
functions:
\begin{align}
  A_x &= |\bm A| \cos\phi\\
  A_y &= |\bm A| \sin\phi
