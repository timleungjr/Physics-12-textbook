...
\gentitle{15}{FLUID MECHANICS}

%\vspace{.15in}
%\fcolorbox{black}{yellow!10}{
%  \begin{minipage}{.98\linewidth}
%    \large
%    \textbf{For all multiple-choice questions in this homework, please answer
%      them directly here on the Classkick selection box. The printed copy
%      (downloaded from the school website) is for reference only. For questions
%      that have two correct answers, please use the highlighter mode to select
%      the second correct answer, as Classkick does not have the capability for
%      multiple selections.}
%  \end{minipage}
%}
%\vspace{.1in}
...
\begin{questions}  
  ...
  Each block is half submerged, as shown below. Water has a density of
  ...
  by a cork. All cylinders are open to the atmosphere at the top. The corks are
  removed. Which of the following is the correct ranking of the velocity of the
  water ($v$) as it exits each cylinder?
  \cpic{.5}{mc-q2}
  \begin{choices}
  ...
  \question A \SI1{\centi\metre} diameter pipe leads to a showerhead with
  twenty \SI1{\milli\metre} diameter exit holes. The velocity of the water in
  the pipe is $\varv$. What is the velocity of the water exiting the holes?
  \begin{choices}
    \choice $0.05\varv$
    \choice $0.5\varv$
    \choice $5\varv$
  ...
  \question\vspace{-.1in}Which of the following is the correct ranking of the
  pressure ($P$) at the bottom of the containers?
  \label{cyl1}
  \begin{choices}
  ...
  \question The force on the bottom of container $A$ due to the fluid inside
  ...
  fluid can pass between the cylinders, as shown in the figure. The cylinder
  on the right has 4 times the diameter of the cylinder on the left. Both
  cylinders are fitted with a movable piston and a platform on top. A person
  stands on the left platform. Which of the following lists the correct
  number of people that need to stand on the right platform so neither
  platform moves. Assume that the platform and piston have negligible mass
  and that all the people have the same mass.
  \cpic{.25}{mc-q5}
  \begin{choices}
    \choice 16 people
    \choice 4 people
  ...
  \question A mass $m$ is suspended in a fluid of density $\rho$ by a string,
  as shown in the figure below. The tension in the string is $T$. Which of the
  ...
  \begin{choices}
    \choice$F_b=mg$
    \choice$F_b=mg-T$
    \choice$F_b=a^2\rho gh_1$
    \choice$F_b=a^2\rho g(h_1-h_2)$
  \end{choices}
...
%  \end{choices}
  ...
  \question A glass U-tube with a uniform diameter of \SI{.850}{\centi\metre}
  ...
  \begin{parts}
    ...
    %The question continues on the next page\ldots
    \newpage
  ...
  \question A \SI{1.0}{\centi\metre} radius hose with a \SI{.50}{\centi\metre}
  radius exit nozzle is being used to fill a \SI1{\litre} beaker with oil
  (\SI1\litre=\SI{.001}{\metre\cubed}). The velocity of
  the oil in the hose is $\varv=\SI{.40}{\metre\per\second}$ as shown in the
  ...
  \question Tank X is a large cylindrical tank that is partially filled with
...
%  \question A large pan is filled to the top with oil of density $\rho_o$. A
...
%  }
%  
%  \question A cube of lead with a side dimension of \SI{5.0}{\centi\metre} is
