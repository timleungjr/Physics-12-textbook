...
must be the same (i.e.\ $V_1=V_2=V_3=\cdots=V$), therefore we can factor out
the voltage term in Eq.~\ref{eq:parallel1}, and re-write in terms of an
\emph{equivalent} resistance $R_p$:
\begin{equation*}
  I=V\left(\frac1{R_1}+\frac1{R_2}+\frac1{R_3}\cdots\right)=
  \frac V{R_p}
\end{equation*}
From this, we find the parallel equivalent resistance of $N$ resistors connected
in parallel: The inverse of the equivalent resistance $R_p$ is the sum of the
inverses of the individual resistances $R_i$:
\begin{important-equation}
...
Also, the parallel equivalent resistance is \emph{always} lower than the 
...
%Increasing the number of resistors in parallel \emph{decreases} $R_p$ because
%it effectively increases the cross section area that the current passes through.
%The parallel equivalent resistance is \emph{always} lower than the lowest
%resistor.
%\begin{figure}[ht]
%  \centering
